        ...
        <div className="space-y-4">
          {transfers.length === 0 ? (
            ...
            transfers.map((transfer) => (
              <div 
                key={transfer.id} 
                className="bg-white dark:bg-[#192b33] rounded-lg p-4 shadow"
              >
                <div className="flex justify-between items-center mb-2">
                  <div className="flex items-center">
                    <div className="bg-gray-200 dark:bg-[#233c48] rounded-full p-2 mr-3">
                      <span className="material-symbols-outlined text-slate-800 dark:text-white">
                        {transfer.type === 'transfer' ? 'send' : 'account_balance'}
                      </span>
                    </div>
                    <div>
                  ...
                  <div className="text-right">
                    <p className={`font-bold ${transfer.status === 'completed' ? 'text-green-500' : 'text-slate-800 dark:text-white'}`}>
                      {transfer.sourceAmount ? `-${transfer.sourceAmount.toFixed(2)}` : 'N/A'} {transfer.sourceCurrency || 'USD'}
                    </p>
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                ...
                <div className="flex justify-between text-sm mt-3">
                  <span className="text-gray-500 dark:text-gray-400">
                    Fee: {transfer.fee ? transfer.fee.toFixed(2) : '0.00'} {transfer.sourceCurrency || 'USD'}
                  </span>
                  <span className="text-gray-500 dark:text-gray-400">
