...
const Dashboard: React.FC = () => {
  ...
  const handleRequestMoney = (amount: number, recipient: string, isLink: boolean) => {
    if (isLink) {
      ...
      const link = `${window.location.origin}/pay?amount=${amount}&recipient=${encodeURIComponent(user?.firstName || 'User')}`;
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background-light dark:bg-background-dark flex items-center justify-center">
        <div className="text-lg text-text-primary-light dark:text-text-primary-dark">Loading...</div>
      </div>
    );
  }
        ...
        <main className="flex-1 w-full max-w-7xl mx-auto p-4 sm:p-6 lg:p-10">
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="w-full lg:w-2/3 flex flex-col gap-8">
              <p className="text-4xl font-black leading-tight tracking-[-0.033em]">Welcome back, {user?.firstName || 'User'}!</p>
              ...
              <div className="p-6 @container bg-content-light dark:bg-content-dark rounded-xl shadow-sm">
                <div className="flex flex-col items-stretch justify-start gap-4 sm:flex-row sm:items-center">
                  <div className="flex w-full min-w-72 grow flex-col items-stretch justify-center gap-1">
                    <p className="text-text-secondary-light dark:text-text-secondary-dark text-sm font-normal leading-normal">Available Balance</p>
                    <p className="text-3xl font-bold leading-tight tracking-[-0.015em]">${balance.toFixed(2)} USD</p>
                  </div>
              ...
              <div className="flex flex-col gap-4">
                <div className="flex flex-col sm:flex-row justify-between items-baseline gap-4 px-1">
                  <h2 className="text-[22px] font-bold leading-tight tracking-[-0.015em]">Recent Activity</h2>
                  <button 
                  ...
                  <ul className="divide-y divide-gray-200/80 dark:divide-gray-700/80">
                    {filteredTransfers.length > 0 ? (
                      filteredTransfers.map((transfer) => (
                        <li key={transfer.id} className="p-4 flex items-center justify-between gap-4 hover:bg-background-light dark:hover:bg-background-dark/50 transition-colors">
                          <div className="flex items-center gap-4">
                            <div className="bg-center bg-no-repeat aspect-square bg-cover rounded-full size-10" 
                              style={{backgroundImage: 'url("https://lh3.googleusercontent.com/aida-public/AB6AXuAuHX-31OOI9vwcX77AokzoeEia8aJjisXNFT_pCzqi0hwVsCiLdeT6eNUnqYPhvx7d16l0SX-V793iW9Wa31bQICo1qLIBAQ-SDrNsaKoMLAtvgvI2poJzFvb3lGujYwgOJX5USjXBsavayMzjWdt-lAxC8KCFr7r7dCo1H5Eg9d50piz6TsjkpmMaJGtuOz6XV_CwiAqK1B68fFPvhzXM4QpIxAdzeU-CtJDzqhm0FO5dQX_vSYLpQ2W329UBsmsmWG0-Pr1d8lY")'}}>
                            </div>
                            <div>
                          ...
                          <p className="font-semibold">
                            {transfer.status === 'completed' ? '-' : ''} ${transfer.sourceAmount.toFixed(2)}
                          </p>
                        </li>
                      ))
                    ) : (
                ...
                <div className="flex flex-col gap-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="bg-center bg-no-repeat aspect-square bg-cover rounded-full size-10" 
                        style={{backgroundImage: 'url("https://lh3.googleusercontent.com/aida-public/AB6AXuB_1zEUTEf596tqyxfQkzuZjVa2gBP36XsrLDAr3IxKXT79t-S6HI6a_N-PZ5C4FNDX4M0XxlpYMdAMU7U4eKDebdngasVYtJtbJhktAjq7h4rj2DlOfnOpKTa89a-uGcm_VNqFToOXM25FpcKN_CWk4poFVo9rA3FeAa982WIBWzKfzdfwVJP03lFnBIJYlSPFl9q0bO_SVgfW6wyessAF0XF1gMn4PtVEVLHH-3jVQz4LOQRVwadQmZPJBsgGTagE2W-ZjWMTeF0")'}}>
                      </div>
                      <span className="font-medium">Jane Doe</span>
                    </div>
                    <button className="text-primary text-sm font-medium hover:underline">Pay</button>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="bg-center bg-no-repeat aspect-square bg-cover rounded-full size-10" 
                        style={{backgroundImage: 'url("https://lh3.googleusercontent.com/aida-public/AB6AXuCu3BzvK2mGWdstU9m5XlCRzn_f7i1iqBEI-tHo4IXJJHQ3fwKRv8TkFnj-5-rr-_DbqK3q3oxBIxBIwMer3rdIgCL0ZTONBkN9UF7dDjW0lWOv7WEDFoGUkJC2HX2d_E1FKy9fscVPUXWV2brTcbx5LMXaEaydB5hfj-cLSzew2YTkWQRHSoVVVhZi6wWcQDR0jqGlc_wUr8Vmc_8W-Ku_FVE5CyTZ-g1ggFeqa8vnElV64HkHOxrhFUHqeUi03ByxWUCi3-UIBM8")'}}>
                      </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 rounded-full size-10 flex items-center justify-center font-bold">AC</div>
